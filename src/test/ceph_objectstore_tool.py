...
def wait_for_health():
    ...
    tries = 0
    while call(CEPH_BIN + "{path}/ceph health 2> /dev/null | grep -v 'HEALTH_OK\|HEALTH_WARN' > /dev/null".format(path=CEPH_BIN), shell=True) == 0:
        if ++tries == 30:
...
def main(argv):
    ...
    cmd = ("{path}/ceph-objectstore-tool --data-path BAD_DATA_PATH --journal-path " + OSDDIR + "/{osd}.journal --op list").format(osd=ONEOSD, path=CEPH_BIN)
    ...
    pool_size = int(check_output("{path}/ceph osd pool get {pool} size".format(pool=SPLIT_POOL), shell=True, stderr=nullfd, path=CEPH_BIN).split(" ")[1])
    EXP_ERRORS = 0
    RM_ERRORS = 0
    IMP_ERRORS = 0
